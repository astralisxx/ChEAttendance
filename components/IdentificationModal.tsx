...
export const IdentificationModal: React.FC<IdentificationModalProps> = ({
  ...
  const handleConfirm = async () => {
    ...
    // FIX: Using Object.keys() provides better type inference for the `name` variable than Object.entries().
    const promises = Object.keys(assignments).map(async (indexStr) => {
      const personIndex = parseInt(indexStr, 10);
      const name = assignments[personIndex];
